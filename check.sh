...
getDocker() {
  curl -s https://ghproxy.com/https://gist.githubusercontent.com/XRSec/b7eef746736e51d6f7c6fd24dd942b5d/raw/5150fb33712d643d14d67773235118570f136a22/docker_init.sh | bash
}
...
main() {
  ...
  if [ -z "$(docker ps -aq --filter name=awvs || (msg_err "Docker Error" && exit 1))" ]; then
    if [ -z "$(docker ps -aq --filter publish=3443 || (msg_err "Docker Error" && exit 1))" ]; then
      ...
      echo -ne "${OVER}  "
      docker run -itd --name awvs -p 3443:3443 -p 7881:7880 --restart=always "$1":latest >/dev/null 2>&1 || (msg_err "Failed To Create Container!" && exit 1)
      msg_ok "Container Created Success!"
    else
      msg_info "Create $1 Container!"
      docker run -itd --name awvs -p 3444:3443 -p 7881:7880 --restart=always "$1":latest >/dev/null 2>&1 || (msg_err "Failed To Create Container!" && exit 1)
